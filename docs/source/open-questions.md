...
### 段階 2 での扱い (2026-09-11、T-2.3〜2.7 で決定・実装済み)

**VS Code 拡張のセッションは段階 2 の集計母集団に入れない** (ADR-0019)。理由は (b) の実測数字そのもの — cwd は kind:0 で 0 件・全行走査で 9/26、ハッシュ再計算は 6/20、終了時刻フィールドが無く FR-P-57 の稼働判定が作れない。`CopilotUsage` は Copilot CLI の `session-state` のみから作る。

**残件 (この OQ を「確定」にするために必要なこと)**:

1. `kind:1` / `kind:2` パッチ行の形式 (JSON Patch か独自形式か)。cwd 逆引きを主経路にするならこれが先
2. `chatSessions` からクレジット / トークンが取れるか (FR-P-50 の項目が埋まるか)
3. 未検証の別案: `<hash>/workspace.json` の folder (20/20 存在) からワークスペースのパスを取り、`chatSessions/` の有無と件数だけを紐付ける経路。日時は `creationDate` 止まり、消費クレジットは取れない
4. ハッシュ不一致 14 件の原因 (VS Code バージョン差 / 多重ルート / `vscode-remote://`)

**実機確認 (2026-09-11)**: ユーザーの実 `~/.copilot/session-state` に 49 件のセッションが実在し、うち 6 件が既存プロジェクト (agent-deck) に完全一致で紐付き、43 件が未紐付けとして正しく警告表示された。INV-1 (mtime とサイズが実行前後で不変) を確認済み。
